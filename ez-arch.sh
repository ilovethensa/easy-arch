...
if [[ -e /sys/firmware/efi/efivars ]] ; then
    info_print "UEFI mode OK."
else
    error_print "System not booted in UEFI mode!"
    exit 1
fi
echo -e "\n"
...
if [[ $? -ne 0 ]] ; then
    error_print "Please check the internet connection."
    exit 1
else
    info_print "[+] Internet OK."
fi


info_print "Updating the system clock"
timedatectl set-ntp true

info_print "EFI boot settings"
efibootmgr
efi_boot_id=" "
